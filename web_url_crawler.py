...
def discover_links_with_depth(start_url, max_depth=4):
    """
    Crawls a website with URL parameter support to a specific depth, ignoring common file types.
    """
    # The queue stores tuples of (URL, current_depth)
    queue = deque([(start_url, 0)])
    
    # This set stores every unique URL string we've already decided to visit to prevent duplicates
    visited_urls = {start_url}
    
    # This set will store the final list of all URLs found
    ...
    print(f"✅ Correctly handling URL parameters (e.g., ?dept=CSE)")
        ...
        try:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if link_domain == base_domain and clean_url not in visited_urls:
                    ...
                    print(f"  [{current_depth + 1}] Discovered: {clean_url}")

                    # Only add the URL to the queue if it's not a file
                    if not clean_url.lower().endswith(ignored_extensions):
...
if __name__ == '__main__':
    ...
    output_filename = "nita_discovered_urls.txt"
    with open(output_filename, "w") as f:
        for url in sorted(list(all_site_urls)):
            f.write(f"{url}\n")
    
    print(f"All discovered URLs have been saved to {output_filename}")